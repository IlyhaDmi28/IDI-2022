...
namespace IT {
	...
	bool SetValue(IT::Entry* entry, char* value) // установка значения переменной
	{
		...
		if (entry->iddatatype == INT)
		{
			...
			else if (value[strlen(value) - 1] == 'b')
				temp = strtol(value, NULL, 2);
			else
				temp = atoi(value);
		...
		else if (entry->iddatatype == BOL)
		{
			...
			for (short i = 0; i < strlen(value); i++)
			{
				if (value[i] != TRUE[i]) IsTrue = false;
				if (value[i] != FALSE[i]) IsFalse = false;
			}

			if (IsTrue) entry->value.vbol = true;
			if (IsFalse) entry->value.vbol = false;
			/*if (strcmp(value, "true")) entry->value.vbol = true;
			else if (strcmp(value, "false")) entry->value.vbol = false;*/
		}
		return rc;
	}

...
namespace Lex
{
	Graph graphs[N_GRAPHS] =
	{
		...
		{ LEX_STDFUNC, FST::FST(GRAPH_SQRT) },
		...
		bool& lex_ok)							// флаг ошибки(по ссылке)
	{
		...
		else // идентификатор
		{
			...
			if (isFunc) //функции библиотеки
			{
				...
				case IT::STDFNC::F_SQRT:
				{
					strncat(id, "_num", ID_MAXSIZE);
					itentry->idtype = IT::IDTYPE::S;
					itentry->iddatatype = SQRT_TYPE;
					itentry->value.params.count = SQRT_PARAMS_CNT;
					itentry->value.params.types = new IT::IDDATATYPE[SQRT_PARAMS_CNT];
					for (int k = 0; k < SQRT_PARAMS_CNT; k++)
						itentry->value.params.types[k] = IT::SQRT_PARAMS[k];
					break;
				}
							...
							if (lexema == LEX_LITERAL)
							{
								...
								if (idxTI == -1)
								{
									...
									if ((getType(id, in.words[i - 1].word)) == IT::IDDATATYPE::INT)
									{
										...
										else if (curword[strlen(curword) - 2] == 'b')

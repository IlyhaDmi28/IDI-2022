...
namespace Gener
{
	...
	{
		bool gen_ok;
		wchar_t Path[256] = L"D:\\KPO\\Compiler\\IDI_Assembler\\";
		wcsncat(Path, parm.out, wcslen(parm.out));
		ofstream ofile(Path);
		...
		for (int i = 0; i < lex.lextable.size; i++)
		{
			...
			case LEX_IF: // условие
			{
				...
				switch (ITENTRY(i + 2).iddatatype)
				{
				case IT::IDDATATYPE::BOL:

					ofile << "\tmov eax, " << ITENTRY(i + 2).id << endl;
					ofile << "\t.IF eax  ==  1\n";
					break;

				case IT::IDDATATYPE::INT:
					ofile << "\tmov eax, " << ITENTRY(i + 2).id << endl;
					ofile << "\tmov ebx, " << ITENTRY(i + 4).id << endl;
				...
				case IT::IDDATATYPE::STR:
					...
					if (ITENTRY(i + 2).id[0] == 'L')
						ofile << "\tmov esi, offset " << ITENTRY(i + 2).id << endl;
					else 
						ofile << "\tmov esi, " << ITENTRY(i + 2).id << endl;

					if (ITENTRY(i + 4).id[0] == 'L')
						ofile << "\tmov edi, offset " << ITENTRY(i + 4).id << endl;
					else 
						ofile << "\tmov edi, " << ITENTRY(i + 4).id << endl;
					...
					if (LEXEMA(i + 3) == LEX_LOGIC_EQUALS)
						ofile << "\t.IF eax  ==  2\n";
					else if (LEXEMA(i + 3) == LEX_LOGIC_NOT_EQUALS)
						ofile << "\t.IF eax  !=  2\n";
					else

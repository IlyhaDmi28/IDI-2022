...
namespace Semantic
{
	bool Semantic::semanticsCheck(Lex::LEX& tables, Log::LOG& log)
	{
		...
		for (int i = 0; i < tables.lextable.size; i++)
		{
			...
			case LEX_EQUAL: //выражение =
			{
				if (i > 0 && tables.lextable.table[i - 1].idxTI != TI_NULLIDX) // левый операнд 
				{
					...
					for (int k = i + 1; tables.lextable.table[k].lexema != LEX_SEPARATOR; k++) //пока не дойдем до ;
					{
						...
						if (lefttype == IT::IDDATATYPE::STR || lefttype == IT::IDDATATYPE::BOL) // справа только литерал, ид или вызов строковой ф-ции
						{
			...
			case LEX_IF:
			{
				if (tables.idtable.table[tables.lextable.table[i + 2].idxTI].iddatatype != IT::IDDATATYPE::BOL
					&& tables.lextable.table[i + 3].lexema == LEX_RIGHTTHESIS)
				{
					// неизвестный ид в условии
